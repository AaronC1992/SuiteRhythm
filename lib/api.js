...
function normalizeSoundRecord(f) {
    const rawType = f?.type;
    const normalizedType = rawType === 'music' ? 'music'
        : rawType === 'ambience' ? 'ambience'
        : 'sfx';
    const src = f?.file || f?.src || '';
    const name = f?.name || src;
