...
const CONTEXT_REQUIRED = {
    ...
    door: /\b(door|gate)\b.{0,40}\b(knock|knocks|knocked|knocking|slam|slams|slammed|creak|creaks|creaked|creaking|open|opens|opened|shut|shuts)\b|\b(knock|knocks|knocked|knocking|slam|slams|slammed|creak|creaks|creaked|creaking|open|opens|opened|shut|shuts)\b.{0,40}\b(door|gate)\b/,
...
export function shouldTriggerKeyword(keyword, transcript = '') {
    const normalizedKeyword = String(keyword || '').toLowerCase().trim();
    const rule = CONTEXT_REQUIRED[normalizedKeyword];
    if (!rule) return true;
    return rule.test(String(transcript || '').toLowerCase());
}
...
export function ruleBasedDecision(transcript, mode, triggerMap, savedSounds) {
    ...
    for (const word of words) {
        ...
        if (match && !triggered.has(match.file) && shouldTriggerKeyword(word, transcript)) {
    ...
    if (musicFiles.length > 0) {
        ...
        const hasTranscriptMusicEvidence = bestTranscriptScore >= 1 || result.sfx.length > 0;
