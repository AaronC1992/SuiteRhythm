    ...
    __authTokenPromise = (async () => {
        try {
            ...
            if (token) {
                ...
                // (engine reads localStorage.SuiteRhythm_token for TTS Bearer header)
                try { localStorage.setItem('SuiteRhythm_token', token); } catch (_) {}
            }
            return __authToken;
        } catch (e) {
        ...
        if (resp.ok) {
            ...
            if (Array.isArray(data?.files)) {
                ...
                __soundsCache = data.files.map(f => {
                    // Preserve the 'ambience' type so the engine's ambient-bed
                    // layer can distinguish looping environmental beds from
                    // one-shot SFX. Anything non-music/non-ambience is sfx.
...
/**
 * Analyze transcript via the SuiteRhythm backend (requires active subscription token)
