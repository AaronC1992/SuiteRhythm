...
function buildFfmpegArgs({ mediaPath, cueAssets, outputPath, outputType, outputFormat }) {
  const args = ['-y', '-i', mediaPath];
  cueAssets.forEach((asset) => args.push('-i', asset.filePath));
  ...
  cueAssets.forEach(({ cue }, assetIndex) => {
    ...
    let filter = `[${inputIndex}:a]aresample=48000,asetpts=PTS-STARTPTS,volume=${volume}`;
