 ...
 * to drive Effexiq without touching its internals.
 *
 ...
 *
 *   1. Direct JS:       window.Effexiq.trigger('thunder')
 *   2. window events:   window.dispatchEvent(new CustomEvent('effexiq:command',
 *                         { detail: { type: 'trigger', query: 'thunder' } }))
 *   3. postMessage:     window.postMessage({ effexiq: 'trigger', query: 'thunder' }, '*')
 *                       (useful for OBS browser-source bridges embedding
 *                       Effexiq in an <iframe>)
 *
 * Plus an anonymous Twitch IRC client that forwards !bang-commands
 * from chat to the same handler — zero OAuth, read-only.
 *
 * Design notes:
 * - No globals other than `window.Effexiq` (documented, intentional).
...
export class ExternalBridge {
    /**
     * @param {object} engine   Effexiq instance
     * @param {object} [opts]
    ...
    install() {
        ...
        window.Effexiq = Object.assign(window.Effexiq || {}, api);
        ...
        window.addEventListener('effexiq:command', this._eventListener);

        // 3. postMessage channel.
        this._pmListener = (e) => {
            const data = e?.data;
            if (!data || typeof data !== 'object' || !data.effexiq) return;
            if (!this._originAllowed(e.origin)) return;
            const cmd = { type: data.effexiq, ...data };
            delete cmd.effexiq;
    ...
    uninstall() {
        if (!this._installed || typeof window === 'undefined') return;
        this._installed = false;
        try { window.removeEventListener('effexiq:command', this._eventListener); } catch (_) {}
    ...
    async _cmdTrigger({ query, options }, ctx) {
        ...
        if (exact) {
            ...
            url = exact.src ? (typeof encodeURI === 'function' ? encodeURI(exact.src) : exact.src) : null;
