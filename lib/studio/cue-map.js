export const CUE_MAP_VERSION = 2;
...
export function normalizeStudioSound(sound) {
  ...
  const src = String(sound?.file || sound?.src || sound?.dataUrl || '').trim();
  ...
  return {
    ...
    name,
    src,
    tags: Array.isArray(sound?.keywords) ? sound.keywords : Array.isArray(sound?.tags) ? sound.tags : [],
  };
}
